...
| Visitor-funded wallet lane | [`lib/wallet.ts`](./lib/wallet.ts), [`app/api/wallet/create/route.ts`](./app/api/wallet/create/route.ts), [`app/api/wallet/status/route.ts`](./app/api/wallet/status/route.ts) | Create/recover Arc testnet wallets by email, issue wallet tokens, check wallet/Gateway funding, and run agent jobs from a user's own balance. |
...
| Gateway settlement proof model | [`lib/settlement.ts`](./lib/settlement.ts), [`lib/settlement-verifier.ts`](./lib/settlement-verifier.ts), [`lib/payment-reconciliation.ts`](./lib/payment-reconciliation.ts) | Separate Circle Gateway settlement references from Arc EVM transaction hashes, link ArcScan only for real tx hashes, and reconcile stale/missing proof metadata. |
