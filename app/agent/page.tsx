    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      <div className="mx-auto max-w-5xl px-5 py-10">
        ...
        <header className="mb-8">
          <h1 className="text-2xl font-semibold tracking-tight">Autonomous Paying Research Agent</h1>
          <p className="mt-1 text-sm text-zinc-400">
            An AI agent researches a company under a strict ${BUDGET} USDC budget, paying for each source with
            real nanopayments that settle on Arc. The spending <span className="text-zinc-200">judgment</span> is the product.
          </p>
        </header>
          ...
          {topics.map((t) => (
            <button
              ...
              className={`rounded-lg border p-3 text-left transition ${
                topic === t.name ? "border-emerald-500 bg-emerald-500/10" : "border-zinc-800 bg-zinc-900 hover:border-zinc-700"
              } disabled:opacity-50`}
            >
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          ...
          <button
            onClick={runCompare}
            disabled={running}
            className="rounded-md border border-zinc-700 bg-zinc-900 px-4 py-2 text-sm font-medium hover:border-zinc-600 disabled:opacity-50"
          >
        ...
        {(events.length > 0 || running) && (
          <section className="mt-6">
            ...
            <div ref={logRef} className="max-h-72 space-y-1.5 overflow-y-auto rounded-lg border border-zinc-800 bg-zinc-900 p-3">
              {events.map((e, i) => (
